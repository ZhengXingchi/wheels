  ...
  <div class="allPage">
    <div class="searchAll">
      <el-row >
        <el-col :span="24">
          ...
          <el-input v-model="channelName" placeholder="请输入渠道商名称" class='search' style='width:120px;' size="mini" clearable></el-input>
            认证状态:  <el-select v-model="authState" size='mini' class="selectChoice" placeholder="认证状态">
            <el-option label="全部" value=""></el-option>
            <el-option label="未认证" value="0"></el-option>
            <el-option label="已认证" value="1"></el-option>
          </el-select>
            账号状态:  <el-select v-model="accountState" size='mini' class="selectChoice" placeholder="账号状态">
            <el-option label="全部" value=""></el-option>
      ...
      <el-row style="margin-left:70px;">
          机审状态:  <el-select v-model="riskState" size='mini' class="selectChoice" placeholder="机审状态">
            <el-option label="全部" value=""></el-option>
            <el-option label="风控未知" value="0"></el-option>
            <el-option label="风控通过" value="1"></el-option>
            <el-option label="风控拒绝" value="2"></el-option>
            <el-option label="未风控" value="-1"></el-option>
            <el-option label="需要人工干预" value="3"></el-option>
            <el-option label="风控处理中" value="4"></el-option>
          </el-select>
            信审状态:  <el-select v-model="trustState" size='mini' class="selectChoice" placeholder="信审状态">
            <el-option label="全部" value=""></el-option>
          ...
          拨打结果: <el-select v-model="callResult" size='mini' class="selectChoice" placeholder="拨打结果">
          <el-option label="全部" value=""></el-option>
          <el-option label="未处理" value="0"></el-option>
          <el-option label="不需要" value="1"></el-option>
          <el-option label="待认证 " value="2"></el-option>
          <el-option label="未接听" value="3"></el-option>
        </el-select>
          分配状态:<el-select v-model="dispatchState" size='mini' class="selectChoice" placeholder="分配状态">
          <el-option label="全部" value=""></el-option>
      ...
      <el-row >
        <el-col :span="24">
          <el-button type="primary" size="mini" @click='recover' style="float:right;margin:0 5px;">恢复</el-button>
          <el-button type="primary" size="mini" @click='forbidden' style="float:right;margin:0 5px;">禁用</el-button>
          <!-- <el-button type="primary" size='mini' @click='addDialResult' style="float:right;margin:0 5px;">添加拨打结果</el-button> -->
          <el-button type="primary" size='mini' @click='circulationRecord' style="float:right;margin:0 5px;">流转记录</el-button>
          <el-button type="primary" size='mini' @click='dialRecord' style="float:right;margin:0 5px;">拨打记录</el-button>
        </el-col>
      </el-row>
    ...
    <div class="list">
      <el-table
        ...
        <el-table-column
          ...
          >
          <template slot-scope="scope">
            <span style="color:#F56C6C" v-if="scope.row.channelId==0">{{scope.row.channelName}}</span>
            <span  v-if="scope.row.channelId!=0 && scope.row.channelId!=null">{{scope.row.channelName}}</span> 
            <span v-if="scope.row.channelId==null">{{scope.row.channelName}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="phone"
        ...
        <el-table-column
          ...
          width='120'>
          <template slot-scope="scope">
            <el-tag type="warning" v-if="scope.row.riskStatus==0">风控未知</el-tag>
            <el-tag  v-if="scope.row.riskStatus==4">风控处理中</el-tag>
            <el-tag type="success" v-if="scope.row.riskStatus==1">风控通过</el-tag>
            <el-tag type="danger" v-if="scope.row.riskStatus==2">风控拒绝</el-tag>
            <el-tag type="info" v-if="scope.row.riskStatus==-1">未风控</el-tag>
            <el-tag type="danger" v-if="scope.row.riskStatus==3">需人工干预</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="trustState"
...
<script>
import {selectUserList, UserForbidden, UserRecover,listTelephonist,distributionMember,reDistributionMember,selectCallList,addCallResult,listDialDistributeRecordHistory} from '@/API'
  ...
  methods: {
    ...
    dispatchSure(){//确定分配订单
      ...
      distributionMember({
        ...
        if(!res.data.success){
          ...
          this.choiceIdArr.forEach((element)=>{
            this.userList.forEach((item,index)=>{
              if(item.userId == element){
                this.userList[index].operator = this.getTelePhoneNameById(this.telephonist);//话务员
                this.userList[index].dispatchState = "已分配";//分配状态
              }
            })
          })
        }
      ...
      reDistributionMember({
        ...
        if(!res.data.success){
          ...
          this.choiceIdArr.forEach((element)=>{
            this.userList.forEach((item,index)=>{
              if(item.userId == element){
                this.userList[index].operator = this.getTelePhoneNameById(this.telephonist);
              }
            })
          })
        }
    ...
    getList(isOr) {//分页查询会员列表
      ...
      let params = {
        ...
        channelName:this.channelName//渠道商
      }
      ...
      let self=this;
      selectUserList(params).then((data)=>{//分页查询会员列表接口
        ...
        if(data){
          data.list.forEach((element,index) => {
            ...
            switch(data.list[index].authStatus){
              case "0":
                 data.list[index].authStates = "未认证"
                break;
              case "1":
                data.list[index].authStates = "已认证"
                break;
              default:
                data.list[index].authStates = ""
            }
            // switch(data.list[index].riskStatus){//机审状态
            //   case 0:
            //     data.list[index].machineState = "风控未知"
            //     break;
            //   case 1:
            //     data.list[index].machineState = "风控通过"
            //     break;
            //   case 2:
            //     data.list[index].machineState = "风控拒绝"
            //     break;
            //   case -1:
            //     data.list[index].machineState = "未风控"
            //     break;
            //   case 3:
            //     data.list[index].machineState = "需人工确认"
            //     break;
            //   default:
            //     data.list[index].machineState = ""
            // }
            switch(data.list[index].authSecondStatus){//信审状态
              case '0':
                data.list[index].trustState = "未审核"
                break;
              case '1':
                data.list[index].trustState = "审核通过"
                break;
              case '2':
                data.list[index].trustState = "暂时拒绝"
                break;
              case '3':
                data.list[index].trustState = "不通过"
                break;
              case '4':
                data.list[index].trustState = "二次审核"
                break;
              default:
                data.list[index].trustState = ""
            }
            ...
            data.list[index].operator=data.list[index].callAdmin;//话务员
