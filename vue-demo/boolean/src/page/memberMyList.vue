<template>
  <div class="allPage">
    <div class="searchAll">
      ...
      <span>认证状态:</span>
      <el-select v-model="authState" size='mini' class="selectChoice" placeholder="认证状态">
        <el-option label="未认证" value="0"></el-option>
        <el-option label="已认证" value="1"></el-option>
      </el-select>

      <el-select v-model="callResult" size='mini' class="selectChoice" placeholder="拨打结果">
      ...
      <!-- <span>分配状态:</span>
      <el-select v-model="dispatchState" size='mini' class="selectChoice">
      ...
      <!-- <el-button type="" size='mini' style="margin-top:10px;margin-left:300px;" @click='circulationRecord' class="publicMidBtn">流转记录</el-button> -->
      <br>
      <el-button type="primary" size='mini' @click='addDialResult' style="float:right;margin:5px;">添加拨打结果</el-button>
      <el-button type="primary" size='mini' @click='dialRecord' style="float:right;margin:5px;">拨打记录</el-button>
    </div>
...
<script>
import {selectMyAuthUserList, UserForbidden, UserRecover,listTelephonist,distributionMember,selectCallList,addCallResult,listDialDistributeRecordHistory} from '@/API'
  ...
  methods: {
    ...
    getList(isOr) {//分页查询会员列表
      ...
      let params = {
        ...
        userStatus: this.accountState,//用户状态:账号状态
      ...
      let self=this;
      selectMyAuthUserList(params).then((data)=>{//分页查询会员未认证列表接口
        console.log('我的会员列表',data);
        data=data.data.data;
        this.userList = [];
        if(data){
