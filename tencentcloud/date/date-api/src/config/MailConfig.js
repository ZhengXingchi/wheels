...
async function send (sendInfo) {
  ...
  // 判断类型
  if (sendInfo.type === 'register') {
    let info = await transporter.sendMail({
      from: '"认证邮件" <gain_in_your_wit@sina.cn>', // sender address
      to: sendInfo.email, // list of receivers
      subject: `欢迎注册玉山便民网`, // Subject line
      text: `您于${sendInfo.registerTime}在玉山便民网注册，您的验证码是${sendInfo.code},邀请码的过期时间: ${sendInfo.expire}`, // plain text body
      html: `
          <div style="border: 1px solid #dcdcdc;color: #676767;width: 600px; margin: 0 auto; padding-bottom: 50px;position: relative;">
          <div style="height: 60px; background: #393d49; line-height: 60px; color: #58a36f; font-size: 18px;padding-left: 10px;">玉山便民网——欢迎来到玉山便民网官方网站</div>
          <div style="padding: 25px">
            <div>您好，您的验证码为${sendInfo.code}。验证码有效时间30分钟，请在${
        sendInfo.expire
        }之前完成注册：</div>
